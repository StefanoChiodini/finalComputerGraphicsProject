...
vec3 BRDF(vec3 V, vec3 N, vec3 L, vec3 Md, vec3 Ms, float gamma) {
  ...
  vec3 R = reflect(-L, N);  // Compute reflection vector
  float NdotL = dot(N, L);  // Compute dot product of normal and light vector
  vec3 ambient = Md * 0.05;  // Compute ambient term

  //Compute diffuse and specular terms
  vec3 diffuse = Md * clamp(NdotL, 0.0, 1.0);
  vec3 specular = Ms * pow(clamp(dot(V, R), 0.0, 1.0), gamma);

  return ambient + gubo.lightColor.rgb * (diffuse + specular);
